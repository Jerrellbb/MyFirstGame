...
@onready var lablTimer = get_node("%lblTimer")
...
func get_random_item():
	...
	for i in UpgradeDb.UPGRADES:
		if i in collected_upgrades: #find already collected upgrades
			pass
		elif i in upgrade_options: #if upgrad is already an option
			pass
...
func change_time(argtime = 0):
	...
	if get_s < 10:
		get_s = str(0,get_s)
	lablTimer.text = str(get_m, ":", get_s)
